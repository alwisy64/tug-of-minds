...
class Turtlebot:
    ...
    def mindwaveCallback(self, msg):
        ...
        if msg.meditation >= self.meditation_threshold:
            twist.linear.x = 0 # if the user ever has a meditation level greater than the threshold stop turtlebot
	    self.done = 1
	elif msg.meditation == 0:
	    twist.linear.x = 0
        else:
            twist.linear.x = 0.1
	    self.distance += .1
    ...
    def run(self):
        while not rospy.is_shutdown() and self.done == 0 and self.distance < self.max_distance:
